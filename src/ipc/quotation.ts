...
import { getQuotation } from '../db/services/quotations';
import { getSettings } from '../db/services/settings';
import type { QuotationItem, QuotationWithItems, Settings } from '@shared/types';
...
function lineGstAmount(it: QuotationItem): number {
  return round2((it.line_total ?? 0) - (it.taxable_value ?? 0));
}

function buildGstBreakup(items: QuotationItem[]): {
  gst_rate: number;
  taxable: number;
  sgst: number;
  cgst: number;
...
function emptyItemCells(): string {
  return Array.from({ length: 13 }, () => '<td>&nbsp;</td>').join('');
}

function itemRowsHtml(items: QuotationItem[], startIndex: number): string {
  return items
    .map((it, i) => {
...
function buildQuotationHtml(quotation: QuotationWithItems, settings: Settings): string {
  const quoteDate = new Date(quotation.quotation_date);
  const printedDate = [
    String(quoteDate.getDate()).padStart(2, '0'),
    String(quoteDate.getMonth() + 1).padStart(2, '0'),
    String(quoteDate.getFullYear()),
  ].join('-');
  ...
  const toDetails = quotation.customer_name
    ? detailBits(
        ['Ph', quotation.customer_phone],
        ['GSTIN', quotation.customer_gstin],
        ['PAN', quotation.customer_pan],
        ['DL', quotation.customer_dl_no]
      )
    : '';

  const roundOff = saleRoundOff(quotation);
  const words = amountInWords(quotation.total);
  const breakup = buildGstBreakup(quotation.items);

  const sumQty = quotation.items.reduce((s, it) => s + (it.quantity || 0), 0);
  const sumTaxable = round2(quotation.items.reduce((s, it) => s + (it.taxable_value || 0), 0));
  const sumGst = round2(quotation.items.reduce((s, it) => s + lineGstAmount(it), 0));
  const sumAmount = round2(quotation.items.reduce((s, it) => s + (it.line_total || 0), 0));

  const pages = chunkItems(quotation.items);
  ...
  const notesHtml = quotation.notes
    ? `<div class="lbl" style="margin-top:3px">Remarks</div>
       <div class="words">${esc(quotation.notes)}</div>`
    : '';
  ...
  const pageHtml = pages
    .map((pageItems, pageIdx) => {
      ...
      const totalsRow = isLast
        ? `<tr class="sum">
            <td colspan="6" class="c"><b>Total</b></td>
        ...
        : `<tr class="sum">
            <td colspan="13" class="c muted">Continued on next page…</td>
          </tr>`;
      ...
      const footer = isLast
        ? `<tr class="foot">
            <td colspan="2" class="foot-left">
              ...
              ${notesHtml}
              <div class="legal">This is a quotation, not a tax invoice · Subject to local jurisdiction · E. &amp; O. E.</div>
            </td>
            <td class="foot-right">
              <table class="tot">
                <tr><td>Discount</td><td class="num">− ${money(quotation.discount)}</td></tr>
                <tr><td>Taxable</td><td class="num">${money(quotation.subtotal)}</td></tr>
                <tr><td>CGST</td><td class="num">${money(quotation.cgst)}</td></tr>
                <tr><td>SGST</td><td class="num">${money(quotation.sgst)}</td></tr>
                <tr><td>Round Off</td><td class="num">${roundOff >= 0 ? '+' : ''}${money(roundOff)}</td></tr>
                <tr class="net"><td>Net Amount</td><td class="num">₹ ${money(quotation.total)}</td></tr>
              </table>
          ...
          <table class="sheet">
            <tr class="head">
              ...
              <td class="meta">
                ...
                <div>Quotation No: <b>${esc(quotation.quotation_no)}</b></div>
                <div>Date: <b>${esc(printedDate)}</b></div>
              </td>
              <td class="party">
                <div class="tag">To</div>
                <div class="name">${esc(quotation.customer_name || 'Customer')}</div>
                ${quotation.customer_address ? `<div class="addr">${esc(quotation.customer_address)}</div>` : ''}
            ...
            <tr class="grow">
              <td colspan="3" class="pad0">
                <table class="items">
                  <colgroup>
                    <col style="width:4%"/><col style="width:18%"/><col style="width:10%"/><col style="width:6%"/>
                    <col style="width:6%"/><col style="width:7%"/><col style="width:5%"/><col style="width:7%"/>
                    <col style="width:5%"/><col style="width:8%"/><col style="width:5%"/><col style="width:8%"/>
                    <col style="width:11%"/>
                  </colgroup>
                  <thead>
                    <tr>
                      <th>#</th><th>Item</th><th>Mfg</th><th>Pack</th>
                      <th>HSN</th><th>MRP</th><th>Qty</th><th>Rate</th>
...
<head>
  <meta charset="utf-8" />
  <title>${esc(quotation.quotation_no)}</title>
  <style>
