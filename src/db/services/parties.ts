...
export function createSupplier(input: SupplierInput): Supplier {
  ...
  const info = db
    .prepare(
      'INSERT INTO suppliers (name, phone, address, gstin) VALUES (@name, @phone, @address, @gstin)'
    )
    .run({
      name: input.name,
...
export function updateSupplier(id: number, input: SupplierInput): Supplier {
  const db = getDb();
  db.prepare(
    'UPDATE suppliers SET name = @name, phone = @phone, address = @address, gstin = @gstin WHERE id = @id'
  ).run({
    id,
    name: input.name,
    phone: input.phone ?? null,
